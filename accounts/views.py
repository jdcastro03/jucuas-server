...
@permission_classes([IsAdminUser])
@transaction.atomic
def change_user_password_with_notification(request, user_id):
    """
    Método para cambiar la contraseña de un usuario especificado por su ID
    y enviar un correo con la nueva contraseña si el checkbox está activado.
    ...
    try:
        with transaction.atomic():
            ...
            notify_user = data.get('notify', False)  # Checkbox para notificar
            ...
            if test_user is None:
                raise Exception("No se puede autenticar con la nueva contraseña")

            # Imprimir información de debug
            print(f"Usuario ID: {user.id}")
            print(f"Contraseña anterior hash: {old_password_hash}")
            print(f"Nueva contraseña hash: {user.password}")
            print(f"Nueva contraseña hash: {user.email}")

            # Enviar correo si el checkbox está activado
            if notify_user:
                try:
                    ...
                    # Eliminar cualquier carácter no ASCII (como emojis y caracteres especiales)
                    message = ''.join(i for i in message if ord(i) < 128)

                    from_email = 'jdnajeracastro38@gmail.com'
                    recipient_list = [user.email]

                    # Usar EmailMessage para tener más control sobre la codificación
                    email = EmailMessage(subject, message, from_email, recipient_list)
                    email.content_subtype = "html"  # Si quieres enviar HTML, si no puedes omitir esta línea
                    email.encoding = 'utf-8'  # Aseguramos que el correo use codificación UTF-8
                    ...
                    return JsonResponse({
                        'status': 'Error',
                        'message': f'Contraseña cambiada, pero no se pudo enviar el correo: {str(e)}'
                    }, status=200)
